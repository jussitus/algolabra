...
class Labyrinth:
    def __init__(self, n_rooms):
        ...
        self.max_dim = 5  # max(5, floor(sqrt(max_rooms)))
        self.min_dim = max(2, self.max_dim // 3)
        self.gap = 1
        self.modifier = 1
        self.max_tries = 100
    ...
    def generate_rooms(self, n):
        rooms = []
        room_centers = []
        total = floor(n * self.max_dim * self.modifier)
        ...
        while len(rooms) < n:
            ...
            valid = True
            corner = point_in_circle(total - self.max_dim, total - self.max_dim)
            width = randint(self.min_dim - 1, self.max_dim - 1)
            height = randint(self.min_dim - 1, self.max_dim - 1)
            for w in range(width + self.gap):
                for h in range(height + self.gap):
